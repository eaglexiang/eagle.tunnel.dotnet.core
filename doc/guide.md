...
本程序只提供代理服务本身的功能，并不会自动配置您的系统代理，也就是说这需要您手动进行设置。系统的自动配置在带图形界面的版本（即[Windows版本](https://github.com/eaglexiang/eagle.tunnel.dotnet)）中提供支持。假如你是萌新，不知道怎么给系统用上刚刚搭建好的代理，建议百度类似`Windows设置HTTP代理`的关键词。在我们设置系统代理的时候，常会用到`代理地址`和`端口号`这两个东西，可从配置文件中找到。例如在本文示例配置中的`VPS的IP:8080`中，`VPS的IP`就是代理地址，`8080`就是端口号。
